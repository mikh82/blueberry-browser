...
export const AddressBar: React.FC = () => {
  ...
  const [isBerryFlowOPen, setIsBerryFlowOpen] = useState(false);
  ...
  const toggleBerryFlow = () => {
    setIsBerryFlowOpen(!isBerryFlowOPen);
    // Send IPC event to toggle sidebar
    if (window.topBarAPI) {
      window.topBarAPI.toggleBerryFlow();
    }
  };
